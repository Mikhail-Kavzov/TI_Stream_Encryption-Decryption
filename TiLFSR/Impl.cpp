...
namespace LFSR {
	...
	void ParseRegString(string& str) //check Register initial string
	{
	...
	udlong StrToDecimal(string& str) //translate string register to decimal number
	{
		udlong result = 0;
		int len = str.length();
		for (int i = 0; i < len; i++)
	...
	void DefinePosCoefs(vector<udlong>& koefs) // defining positions of  bits for editing
	{
	...
	udlong DefineZeroBit(udlong Reg, vector<udlong>& koefs) //implementation algorithm
	{
		udlong result;
	...
	unsigned char TransfromReg(udlong& Reg, vector<udlong>& koefs) //implementation algorithm
	{
	...
	String^ ToBinaryString(unsigned char val, String^ str) //converting value to binary string
	{
